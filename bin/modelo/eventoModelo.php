      ...
      private function actualizarStockYReservado($idAlimento, $cantidad) {
        ...
        private function notificaciones2() {
          try {
            ...
            $query = $this->conex2->prepare("SELECT * FROM usuario u INNER JOIN rol r ON u.idRol = r.idRol INNER JOIN permiso p ON p.idRol = r.idRol INNER JOIN modulo m ON m.idModulo = p.idModulo WHERE m.nombreModulo = 'Eventos' and p.nombrePermiso = 'consultar' and p.status = 1 and u.status = 1;");
            $query->execute();

  ...
  let mostrarS = $('.tabla').DataTable({
    "columns": [
        ...
        { "data": "descripcion" },
        {
            ...
            "render": function(data) {
                return `
                <a id="${data}" class="btn btn-sm btn-icon text-info flex-end text-center informacion" data-bs-toggle="modal" data-bs-target="#infoSAlimento" data-bs-toggle="tooltip" title="informacion Alimento" href="#">
                ...
                <a id="${data}" class="btn btn-sm btn-icon text-primary text-center pdf" data-bs-toggle="modal" data-bs-target="#pdfSAlimento" data-bs-toggle="tooltip" title="Descargar Salida de Alimentos" href="#" type="button">
...
function tablaSalidaAlimentos() {
    ...
    $.ajax({
        ...
        url: "", 
        dataType: "json",
        data: { mostrarSalidaA: true, fechaInicio, fechaFin },
        success(data) {
            $('#ani').show(2000);
            mostrarS.clear().rows.add(data).draw();
            mostrarS.on('draw.dt', function () {
            quitarBotones();
             });
             if (data.length > 0) {
               quitarBotones();
      ...
      if (data.resultado === "no se puede"){
         ...
         $('#cerrar3').click();
             Swal.fire({
               ...
               Estos alimentos ya fueron utilizados en el Servicio Nutricional. `,
       ...
       if (data.resultado === 'ya no existe') {
        $('#cerrar3').click();
         ...
         Swal.fire({
                ...
                position: 'top-end',
                icon:'error',
                title:'<span class=" text-rojo">La salida de alimentos fue anulado recientemente!</span>',
                showConfirmButton:false,
                timer:3000,
                timerProgressBar:3000,
            })
              ...
              valAnulacion(id);
              $('.eliminarI').html( '¿Deseas anular esta salida de alimentos ?');
      ...
      if (data.mensaje.resultado === 'eliminado' && data.newCsrfToken){
        ...
        tablaSalidaAlimentos();
          Swal.fire({
               ...
               position: 'top-end',
               icon:'success',
               title:'Salida de Alimentos Anulado Exitosamente!',
               showConfirmButton:false,
               timer:2500,

...
function quitarBotones() {
  ...
  if (
    typeof permisos.eliminar == "undefined" &&
    typeof permisos.modificar == "undefined"
  ) {
    console.log(permisos);
    $(".accion").remove();
...
function danger() {
  $(".error1").html(
    ' <i  class="bi bi-exclamation-triangle-fill"></i>¡El tipo de Salida ya existe!'
  );
  $(".error1").show();
...
function val_tipoS() {
  ...
  if (chequeo.test(nombre) && nombre !== "") {
    ...
    $(".error1").html(
      '<i  class="bi bi-exclamation-triangle-fill"></i>¡Ingrese el tipo de Salida, solo Caracteres!'
    );
    $(".error1").show();
    $("#tipoS").addClass("errorBorder");
    $(".bar1").removeClass("bar");
    ...
    success(data) {
      if (data.resultado == "error tipo") {
        Swal.fire({
          ...
          icon: "error",
          title:
            'El tipo de Salida <b class="fw-bold text-rojo">' +
            tipoS +
            "</b> ya esta registrado, ingrese otro tipo de Salida!",
          showConfirmButton: false,
          timer: 3000,
          timerProgressBar: 3000,
        });
...
function registrar() {
  ...
  if (token) {
    $.ajax({
      ...
      url: "",
      dataType: "JSON",
      data: { registrar: true, tipoS, csrfToken: token },
      success(data) {
        // Verifica los datos que recibes del servidor
        if (data.mensaje.resultado == "registrado" && data.newCsrfToken) {
          ...
          Swal.fire({
            ...
            icon: "success",
            title:
              'El tipo de Salida <b class="text-primary fw-bold">' +
              tipoS +
              "</b> Registrado Exitosamente!",
            showConfirmButton: false,
            timer: 2500,
    ...
    success(data) {
      if (id == 1) {
        ...
        Swal.fire({
          ...
          icon: "error",
          title:
            '<span >¡El Tipo de Salida <b class=" text-rojo" >Menú</b> no puede ser modificado!</span>',
          showConfirmButton: false,
          timer: 3000,
          timerProgressBar: 3000,
        });
...
function modificarTipoSalida() {
  ...
  if (token) {
    $.ajax({
      ...
      method: "POST",
      dataType: "json",
      data: { tipoS2, id, csrfToken: token },
      success(data) {
          ...
          if (data.resultado === "error tipo") {
            Swal.fire({
              ...
              title:
                'El Tipo de salida <b class="fw-bold text-rojo">' +
                tipoS2 +
                "</b> ya está registrado, ingrese otro tipo de salida!",
              showConfirmButton: false,
              timer: 3000,
              timerProgressBar: 3000,
            });
            $("#error2").html(
              ' <i  class="bi bi-exclamation-triangle-fill"></i> El salida ya existe!'
            );
            danger1();
          } else {
    ...
    success(data) {
      if (id == 1) {
        ...
        Swal.fire({
          ...
          icon: "error",
          title:
            '<span >¡El Tipo de Salida <b class=" text-rojo" >Menú</b> no puede ser anulado!</span>',
          showConfirmButton: false,
          timer: 3000,
          timerProgressBar: 3000,
        });
  ...
  $.ajax({
    ...
    success(data) {
      if (data[0].tipoSalida != "Menú") {
        $(".eliminarTS").html(
          '¿Deseas Anular <b class="text-primary">' +
            data[0].tipoSalida +
            "</b> del tipo de Salida?"
        );
      }
    },
  });
      ...
      success(data) {
        ...
        if (data.resultado === "ya no existe") {
          ...
          Swal.fire({
            ...
            icon: "error",
            title:
              '<span class=" text-rojo">¡El Tipo de Salida fue anulado recientemente!</span>',
            showConfirmButton: false,
            timer: 3000,
            timerProgressBar: 3000,
          });
